...
#[cfg(flash_g4c4)]
#[inline(always)]
fn set_pnb_c4(cr: &mut pac::flash::regs::Cr, val: u8) {
    cr.0 = (cr.0 & !(0xff << 3usize)) | (((val as u32) & 0xff) << 3usize);
}

#[inline(always)]
fn set_page_number(cr: &mut pac::flash::regs::Cr, sector: &FlashSector) {
    #[cfg(flash_g0x0)]
    cr.set_pnb(sector.index_in_bank as u16);
    #[cfg(flash_g4c4)]
    set_pnb_c4(cr, sector.index_in_bank as u8);
    #[cfg(not(any(flash_g0x0, flash_g4c4)))]
    cr.set_pnb(sector.index_in_bank as u8);
}
