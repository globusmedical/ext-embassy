...
impl<'d, T: GeneralInstance4Channel> Timer<'d, T> {
    ...
    /// Generate an update event (UG) by software.
    pub fn generate_update_event(&self) {
        self.regs_gp16().egr().write(|r| r.set_ug(true));
    }
}
...
impl<'d, T: AdvancedInstance4Channel> Timer<'d, T> {
    ...
    /// Generate a COM event (COMG) by software.
    pub fn generate_com_event(&self) {
        self.regs_advanced().egr().write(|r| r.set_comg(true));
    }
}
